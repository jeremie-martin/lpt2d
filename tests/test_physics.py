...
# ---------------------------------------------------------------------------
# Material defaults (all 12 fields required by the strict JSON parser)
# ---------------------------------------------------------------------------
...
def _complete_material(mat: dict) -> dict:
    """Return a copy of mat with all missing fields filled from defaults."""
...
def test_snell_law():
    ...
    # Beam from (0, 0.7) along (sin theta_i, -cos theta_i) hits slab top at y=0.15
    t_entry = (0.7 - slab_top) / math.cos(theta_i)
    x_entry = 0.0 + math.sin(theta_i) * t_entry

    # Inside slab, travels at theta_r from vertical for thickness 0.3
    x_exit = x_entry + (slab_top - slab_bot) * math.tan(theta_r)

    # After exit, continues at original theta_i angle
    y_measure = -0.5
...
def test_tir():
    ...
    #
    # A beam entering the bottom face vertically hits the hypotenuse at 45 deg.
    # For ior=1.5: critical angle = 41.8 deg. Since 45 deg > 41.8 deg -> TIR.
    # For ior=1.2: critical angle = 56.4 deg. Since 45 deg < 56.4 deg -> transmission.
    #
