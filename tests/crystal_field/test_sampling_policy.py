...
def test_active_free_sampler_temporarily_excludes_glass():
    ...
    assert DEFAULT_SAMPLER_POLICY.outcomes == (
        ("black_diffuse", 1.0),
        ("gray_diffuse", 1.0),
        ("colored_diffuse", 1.0),
        ("brushed_metal", 3.0),
    )
    total_weight = sum(weight for _outcome, weight in DEFAULT_SAMPLER_POLICY.outcomes)
    brushed_weight = dict(DEFAULT_SAMPLER_POLICY.outcomes)["brushed_metal"]
    assert brushed_weight / total_weight == 0.5
...
def test_diffuse_light_speed_is_faster_than_brushed_metal_from_same_base_draw():
    rng_diffuse = random.Random(41)
    rng_metal = random.Random(41)

    diffuse = _sample_light_speed(
        rng_diffuse,
        n_lights=1,
        outcome="gray_diffuse",
        policy=DEFAULT_SAMPLER_POLICY.light,
    )
    metal = _sample_light_speed(
        rng_metal,
        n_lights=1,
    ...
    assert diffuse > metal
    assert diffuse == pytest.approx(
        metal
        * (
            DEFAULT_SAMPLER_POLICY.light.diffuse_speed_multiplier
            / DEFAULT_SAMPLER_POLICY.light.brushed_metal_speed_multiplier
        )
    )
