...
@pytest.mark.parametrize(
    ("variants", "duplicate_name"),
    [
        ([{"exposure": -1.0}, ("variant_000", {"gamma": 0.9})], "variant_000"),
        ({1: {"exposure": -1.0}, "1": {"gamma": 0.9}}, "1"),
    ],
)
def test_render_frame_variants_rejects_duplicate_normalized_names(
    monkeypatch, variants, duplicate_name
):
    called = False
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            "render_frame_variants() requires unique variant names after normalization: "
            f"{duplicate_name!r}"
        ),
    ):
        renderer_mod.render_frame_variants(
            animate,
            Timeline(1.0, fps=1),
            settings=Shot(canvas=Canvas(10, 8), look=Look(exposure=-4.0, gamma=2.0)),
            variants=variants,
