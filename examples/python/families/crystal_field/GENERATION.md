...
brushed metal weighted to 50% and the three diffuse outcomes sharing the
remaining 50%:
...
For active paths, `speed` is the authored motion contract: world units per
second. Scene assembly converts it to a path budget with
`total_distance = speed * duration`, then the path is converted to linear
arc-length tracks over the clip. A 20 second clip therefore travels twice as
far as a 10 second clip at the same sampled speed.

The route builders keep that distance budget but avoid visually cramped paths:

- `channel` prefers farther corridor destinations and avoids immediately
  stepping back onto the previous edge when another route is available.
- `drift` keeps larger free-flight legs and rejects near-180-degree direction
  flips unless the bounds force them. Scene assembly also scores drift
  candidates against the actual object geometry and prefers a candidate whose
  path-length fraction inside objects stays below 35%, while still allowing
  occasional object crossings.

Speed is sampled from the same base range for all outcomes, then shaped by the
material: diffuse scenes move 20% faster, while brushed metal moves 10% slower.
