...
3. **Material** — one of four active peer outcomes is sampled, with brushed
   metal weighted to 50% and the three diffuse outcomes sharing the remaining
   50%.
4. **Light** — 1–3 moving lights at constant speed. Active paths are drift
   and channel only, with channel slightly preferred; the older waypoint,
   random-walk, and vertical-drift styles remain in code with zero sampler
   weight. Speed is authored in world units per second, so longer clips travel
   proportionally farther. Diffuse outcomes get a small speed boost for clearer
   motion; brushed metal is kept slower. Drift paths are selected from several
   candidates when needed so they can pass through objects without spending most
   of their length inside object geometry. When objects have no spectral color,
   warm range spectra (orange/deep-orange) are sampled often. Colored moving
   lights get one shared complementary ambient color with randomized hue jitter
   and white mix; per-light ambient variation is left for future exploration.
...
| Speed | base 0.08–0.20 u/s | Max is 0.20 for one light, 0.14 for multi-light before material shaping; diffuse x1.20, brushed metal x0.90 |
