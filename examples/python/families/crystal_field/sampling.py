...
of **four active peer outcomes** with brushed metal weighted to 50%, then
...
class LightPolicy:
    ...
    diffuse_speed_multiplier: float = 1.20
    brushed_metal_speed_multiplier: float = 0.90
...
def _is_colored_light_spectrum(spectrum: LightSpectrumConfig) -> bool:
    ...
    return max(effective) - min(effective) > 1e-5
...
def _speed_multiplier_for_outcome(outcome: MaterialOutcome, policy: LightPolicy) -> float:
    if outcome == "brushed_metal":
        return policy.brushed_metal_speed_multiplier
    if outcome in {"black_diffuse", "gray_diffuse", "colored_diffuse"}:
        return policy.diffuse_speed_multiplier
    return 1.0


def _sample_light_speed(
    rng: random.Random,
    *,
    n_lights: int,
    ...
    base_speed = rng.uniform(policy.speed_min, speed_max)
    return base_speed * _speed_multiplier_for_outcome(outcome, policy)
...
def _random_light(
    ...
    speed = (
        overrides.speed
        if overrides.speed is not None
        else _sample_light_speed(rng, n_lights=n_lights, outcome=outcome, policy=policy)
    )
