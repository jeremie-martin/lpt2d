...
from .scene import build, rendered_light_intensity, spectrum_intensity_multiplier
...
def _with_white_ambient(p: Params) -> Params:
    white_spectrum = LightSpectrumConfig()
    rendered_intensity = rendered_light_intensity(
        p.light.ambient.intensity,
        p.light.ambient.spectrum,
    )
    ambient = dataclasses.replace(
        p.light.ambient,
        intensity=rendered_intensity / spectrum_intensity_multiplier(white_spectrum),
