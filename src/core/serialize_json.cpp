...
LightSpectrum read_light_spectrum(const Json& json, std::string_view context) {
    ...
    if (type == "range") {
        ...
        return light_spectrum_range(
            read_required_float(json, "wavelength_min", context),
            read_required_float(json, "wavelength_max", context));
    }
    if (type == "color") {
        reject_unknown_keys(json, {"type", "linear_rgb", "white_mix",
                                   "spectral_c0", "spectral_c1", "spectral_c2"}, context);
        const auto rgb = read_rgb(require_key(json, "linear_rgb", context),
                                  std::string(context) + ".linear_rgb");
        const float white_mix = read_required_float(json, "white_mix", context);
        const Json* spectral_c0 = find_key(json, "spectral_c0");
        const Json* spectral_c1 = find_key(json, "spectral_c1");
        const Json* spectral_c2 = find_key(json, "spectral_c2");
        const bool has_any_spectral_coeff = spectral_c0 || spectral_c1 || spectral_c2;
        if (has_any_spectral_coeff && !(spectral_c0 && spectral_c1 && spectral_c2))
            fail(std::string(context) + ".color spectra must provide spectral_c0, spectral_c1, and spectral_c2 together");
        if (!has_any_spectral_coeff)
            return light_spectrum_color(rgb[0], rgb[1], rgb[2], white_mix);

        LightSpectrum spectrum = light_spectrum_from_coeffs(
            read_float(*spectral_c0, std::string(context) + ".spectral_c0"),
            read_float(*spectral_c1, std::string(context) + ".spectral_c1"),
            read_float(*spectral_c2, std::string(context) + ".spectral_c2"));
        spectrum.linear_r = rgb[0];
        spectrum.linear_g = rgb[1];
        spectrum.linear_b = rgb[2];
        spectrum.white_mix = white_mix;
        return spectrum;
    }
...
Json write_light_spectrum(const LightSpectrum& spectrum) {
    if (spectrum.type == LightSpectrumType::Color) {
        return Json{{"type", "color"},
                    ...
                    {"spectral_c0", spectrum.spectral_c0},
                    {"spectral_c1", spectrum.spectral_c1},
                    {"spectral_c2", spectrum.spectral_c2}};
    }
...
Light read_light(const Json& json, std::string_view context, int version) {
    ...
    if (type == "projector") {
        ...
        ProjectorLight light{id, {}, {1.0f, 0.0f}, 0.03f, 0.1f, ProjectorProfile::Uniform, ProjectorSource::Line, 0.0f, 1.0f};
        light.position = read_vec2(require_key(json, "position", context), std::string(context) + ".position");
