...
class Look:
    ...
    _explicit_fields: frozenset[str] = field(default_factory=frozenset, init=False, repr=False, compare=False)
    ...
    def __setattr__(self, name: str, value) -> None:
        if name in _LOOK_FIELDS:
            if name == "background":
                value = list(value)
            object.__setattr__(self, name, value)
            explicit = set(getattr(self, "_explicit_fields", frozenset()))
            explicit.add(name)
            object.__setattr__(self, "_explicit_fields", frozenset(explicit))
            return
        object.__setattr__(self, name, value)
...
def diagnose_scene(scene: Scene) -> list[str]:
    ...
    glass_no_absorb = [
        s
        for s in all_shapes
        if _shape_material(s).transmission > 0.5 and _shape_material(s).absorption < 0.01
    ]
