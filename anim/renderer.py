...
def _validate_unique_variant_names(named_variants: list[_NamedLookVariant]) -> None:
    ...
    if duplicates:
        ...
        raise ValueError(
            f"render_frame_variants() requires unique variant names after normalization: {joined}"
        )
...
def render_frame_variants(
    ...
    *,
    ...
    named_variants = list(_iter_named_look_variants(variants))
    _validate_unique_variant_names(named_variants)
    return {
