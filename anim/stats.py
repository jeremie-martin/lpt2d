...
def diff_image_stats(a: ImageStats, b: ImageStats) -> StatsDiff:
    """Per-field difference `b - a` between two ImageStats."""
    return StatsDiff(
        mean_luma=b.mean_luma - a.mean_luma,
        near_black_fraction=b.near_black_fraction - a.near_black_fraction,
        clipped_channel_fraction=(
            b.clipped_channel_fraction - a.clipped_channel_fraction
        ),
        median_luma=b.median_luma - a.median_luma,
        p95_luma=b.p95_luma - a.p95_luma,
        interdecile_luma_range=(
            b.interdecile_luma_range - a.interdecile_luma_range
        ),
    )
...
def compare_stats(
    ...
    for (fi_a, t_a, sa), (fi_b, _, sb) in zip(a, b, strict=True):
        ...
        diffs.append((fi_a, t_a, diff_image_stats(sa, sb)))
    return diffs
